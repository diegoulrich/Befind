...
async function getStripeCredentials(): Promise<{ secretKey: string; webhookSecret?: string }> {
  ...
  logger.info({ itemCount: data.items?.length, firstKeys: data.items?.[0] ? Object.keys(data.items[0]) : [] }, "Stripe connection API response");

  const item = data.items?.[0];
  const settings = item?.settings;

  const secretKey = settings?.secret_key ?? settings?.secret;
  logger.info({ settingsKeys: settings ? Object.keys(settings) : null, hasSecretKey: !!secretKey }, "Stripe settings inspection");
